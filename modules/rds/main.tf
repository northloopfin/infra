...
# RDS with existing db parameter group with KMS
resource "aws_db_instance" "new_rds_with_kms" {
  count                   = "${length(compact(split(",", var.db_parameter_group_name))) * length(compact(split(",", var.kms_key_arn)))}"
  identifier              = "rds-${terraform.workspace}-${var.client_name}-${var.db_name}"
  allocated_storage       = "${var.db_size}"
  engine                  = "${var.db_engine}"
  engine_version          = "${var.db_engine_version}"
...
# RDS with new db parameter group
resource "aws_db_instance" "new_rds_pg" {
  count                   = "${length(compact(split(",", var.new_parameter_group_name))) * signum(length(compact(split(",", var.kms_key_arn))) - 1) * - 1}"
  identifier              = "rds-${terraform.workspace}-${var.client_name}-${var.db_name}"
  allocated_storage       = "${var.db_size}"
  engine                  = "${var.db_engine}"
  engine_version          = "${var.db_engine_version}"
  ...
  vpc_security_group_ids  = ["${var.sg_id_rds}"]
  db_subnet_group_name    = "${aws_db_subnet_group.rds-subgroup.name}"
  parameter_group_name    = "${aws_db_parameter_group.new_parameter_group.id}"
  storage_encrypted       = "${var.storage_encrypted}"
  tags                    = "${merge(map("env", var.env), var.tags)}"
}

# RDS with new db parameter group with KMS
resource "aws_db_instance" "new_rds_pg_with_kms" {
  count                   = "${length(compact(split(",", var.new_parameter_group_name))) * length(compact(split(",", var.kms_key_arn)))}"
  identifier              = "rds-${terraform.workspace}-${var.client_name}-${var.db_name}"
  allocated_storage       = "${var.db_size}"
  engine                  = "${var.db_engine}"
  engine_version          = "${var.db_engine_version}"
